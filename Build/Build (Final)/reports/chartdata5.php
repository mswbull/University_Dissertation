...
while($row=mysql_fetch_array($result)){
        ...
        echo"<string>".$row['club_association']."</string>";

        $query1 = "select count(eventdetails_result) AS Wins, club_name, eventdetails_result from club, crew, eventdetails WHERE club.club_id = crew.club_id AND crew.crew_id = eventdetails.crew_id AND club.club_association = '".$row['club_association']."' GROUP BY club.club_id, eventdetails_result";
        $result1 = @mysql_query($query1, $connection)
        or die ("Unable to perform query<br />$query1");
                $totalPoints = 0;
